...
public class RagService {
    ...
    public RagChatResponse chat(RagChatRequest request) {
        ...
        String chatModel = request.chatModel() != null && !request.chatModel().isBlank()
                ? request.chatModel().trim()
                : null;
        String answer = chatClient.chat(ragProperties.getSystemPrompt(), userMessage, chatModel);
    ...
    private static SearchRequest toSearchRequest(RagChatRequest request, int topK) {
        ...
        return new SearchRequest(request.libraryId(), request.tenantId(), request.question(), topK, filter);
    }
