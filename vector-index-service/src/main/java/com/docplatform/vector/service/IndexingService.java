...
import com.docplatform.vector.event.VectorEventPublisher;
...
public class IndexingService {
    ...
    private final VectorEventPublisher eventPublisher;
    ...
    public IndexingService(
            ...
            VectorEventPublisher eventPublisher) {
    ...
    public void index(DocumentReadyForIndexEvent event) {
        ...
        try {
            ...
            log.info("Indexed doc {} v{} with {} chunks", docId, event.version(), chunks.size());
        } catch (Exception e) {
    ...
    @Transactional
    public void rebuild(UUID docId, String tenantId, int version, String parsedTextUrl) {
        index(DocumentReadyForIndexEvent.create(
                docId, tenantId, version, "", "text/plain", parsedTextUrl));
    }
    ...
    private void completeJob(DocumentIndexJob job, DocumentReadyForIndexEvent event, int chunkCount) {
        ...
        jobStore.save(job);
        eventPublisher.publish(DocumentIndexedEvent.create(
                event.docId(), event.tenantId(), event.version(), chunkCount));
    }
