...
| 库级目标 | 单库内可配 Parser + Chunk method + Embedding + Retrieval | 单库内：数值 + Embedding + 检索 + 父子块/分隔符；分块策略只读 |

---

## 1.1 产品形态边界（当前版本 · 暂不做）

与主流通用 RAG 对比时，下列能力**刻意不在当前版本范围**，避免与解析 / 分块 / 索引分节职责冲突：

| 能力 | 状态 | 当前替代 |
|------|------|----------|
| **库类型**（Document / FAQ / Web 等） | 暂不做 | 统一「向量知识库」；按 MIME + 内置解析器区分文件 |
| **场景 preset**（paper / book / 高质量档等） | 暂不做 | 快速建库 + 设置页分项配置（解析 / 分块 / 索引 / 检索） |
| **复制 / 克隆库配置** | 暂不做 | 新建库后于设置页手动配置 |
| **索引质量档位**（经济 / 标准 / 高质量） | 暂不做 | 分块参数、解析器、Embedding、检索各自独立配置 |
| **非文件数据源**（URL 抓取、API 同步、FAQ 录入等） | 暂不做 | **仅文件上传链路**（单文件 / 批量 / 文件夹） |
...
按优先级排列，**仍保持「库级主旋钮」原则**。**不含** §1.1 已声明暂不做项（库类型、preset、克隆、索引质量档位、非文件数据源）。
...
| 日期 | 说明 |
|------|------|
| 2026-06-16 | 补充 §1.1 产品形态边界：暂不做库类型 / preset / 克隆 / 索引质量档位；数据源仅文件上传 |
